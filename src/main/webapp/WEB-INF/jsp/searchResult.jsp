...
.productStatus .ww-light a {
	background: url(/img/fore/ww-online.png) center center no-repeat
		!important;
	...
	vertical-align: text-bottom;
			...
			<div class="product">
				<div class="product-iWrap">
					...
					<p class="productStatus">
						<span>销量<em>${p.sale}</em></span> <span>评价<a
							href="#nowhere">${p.reviewCount}</a></span> <span class="ww-light"><a></a></span>
					</p>
				</div>
			</div>
		</c:forEach>
		...
		<style>
.nrt {
	background: url(/img/logo.png) 35px center no-repeat #fff8f6;
	color: #595959;
	margin-bottom: 10px;
	...
	margin-top: 25px;
		...
		<c:if test="${empty products}">
			<div class="nrt">
				<p>
					没找到与 <em>" ${param.keyword} "</em> 相关的商品哦。
				</p>
			</div>
		</c:if>
		<div style="clear: both"></div>
